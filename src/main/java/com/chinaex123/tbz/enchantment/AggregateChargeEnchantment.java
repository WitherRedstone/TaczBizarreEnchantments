...
public class AggregateChargeEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.AGGREGATE_CHARGE_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.AGGREGATE_CHARGE_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.AGGREGATE_CHARGE_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.AGGREGATE_CHARGE_MAX_LEVEL.get();
    }
