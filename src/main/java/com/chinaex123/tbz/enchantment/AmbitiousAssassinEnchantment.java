...
public class AmbitiousAssassinEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZConfig.AMBITIOUS_ASSASSIN_BASE_ENCHANTABILITY.get() + (level - 1) * TBZConfig.AMBITIOUS_ASSASSIN_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZConfig.AMBITIOUS_ASSASSIN_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZConfig.AMBITIOUS_ASSASSIN_MAX_LEVEL.get();
    }
    ...
    public boolean canEnchant(@NotNull ItemStack stack) {
        ...
        if (!GunEnchantmentHelper.canApplyEnchantmentToGunWithWeaponType(
                TBZMod.id("ambitious_assassin"), stack
        )) {
            return false;
        }
