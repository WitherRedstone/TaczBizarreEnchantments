...
public class AttritionOrbsEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.ATTRITION_ORBS_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.ATTRITION_ORBS_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.ATTRITION_ORBS_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.ATTRITION_ORBS_MAX_LEVEL.get();
    }
    ...
    public boolean canEnchant(@NotNull ItemStack stack) {
        ...
        if (!GunEnchantmentHelper.canApplyEnchantmentToGunWithWeaponType(
                TBZMod.id("attrition_orbs"), stack
        )) {
            return false;
        }
