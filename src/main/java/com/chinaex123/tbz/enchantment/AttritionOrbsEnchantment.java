...
public class AttritionOrbsEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZConfig.ATTRITION_ORBS_BASE_ENCHANTABILITY.get() + (level - 1) * TBZConfig.ATTRITION_ORBS_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZConfig.ATTRITION_ORBS_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZConfig.ATTRITION_ORBS_MAX_LEVEL.get();
    }
