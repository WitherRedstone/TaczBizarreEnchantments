...
/** 燃烧野心：造成持续伤害会点燃目标 */
public class BurningAmbitionEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.BURNING_AMBITION_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.BURNING_AMBITION_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.BURNING_AMBITION_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.BURNING_AMBITION_MAX_LEVEL.get();
    }
