...
public class ButterflyEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.BUTTERFLY_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.BUTTERFLY_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.BUTTERFLY_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.BUTTERFLY_MAX_LEVEL.get();
    }
