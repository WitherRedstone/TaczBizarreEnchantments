...
public class ChillClipEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZConfig.CHILL_CLIP_BASE_ENCHANTABILITY.get() + (level - 1) * TBZConfig.CHILL_CLIP_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZConfig.CHILL_CLIP_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZConfig.CHILL_CLIP_MAX_LEVEL.get();
    }
