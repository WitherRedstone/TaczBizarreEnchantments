...
public class ClownCartridgeEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.CLOWN_CARTRIDGE_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.CLOWN_CARTRIDGE_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.CLOWN_CARTRIDGE_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.CLOWN_CARTRIDGE_MAX_LEVEL.get();
    }
