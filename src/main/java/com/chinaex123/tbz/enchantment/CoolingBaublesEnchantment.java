...
public class CoolingBaublesEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZConfig.COOLING_BAUBLES_BASE_ENCHANTABILITY.get() + (level - 1) * TBZConfig.COOLING_BAUBLES_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZConfig.COOLING_BAUBLES_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZConfig.COOLING_BAUBLES_MAX_LEVEL.get();
    }
