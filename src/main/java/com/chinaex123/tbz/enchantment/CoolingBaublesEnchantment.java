...
/** 冷却饰物：收集经验球有概率降低武器热量 **/
public class CoolingBaublesEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.COOLING_BAUBLES_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.COOLING_BAUBLES_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.COOLING_BAUBLES_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.COOLING_BAUBLES_MAX_LEVEL.get();
    }
