...
public class CreepingAttritionEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.CREEPING_ATTRITION_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.CREEPING_ATTRITION_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.CREEPING_ATTRITION_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.CREEPING_ATTRITION_MAX_LEVEL.get();
    }
