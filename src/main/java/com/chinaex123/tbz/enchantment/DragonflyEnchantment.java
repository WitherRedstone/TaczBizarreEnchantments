...
public class DragonflyEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZConfig.DRAGONFLY_BASE_ENCHANTABILITY.get() + (level - 1) * TBZConfig.DRAGONFLY_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZConfig.DRAGONFLY_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZConfig.DRAGONFLY_MAX_LEVEL.get();
    }
