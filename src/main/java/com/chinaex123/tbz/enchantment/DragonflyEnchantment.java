...
/** 蜻蜓：精准击杀可产生爆炸伤害 **/
public class DragonflyEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.DRAGONFLY_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.DRAGONFLY_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.DRAGONFLY_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.DRAGONFLY_MAX_LEVEL.get();
    }
