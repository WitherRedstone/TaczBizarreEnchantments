...
public class ExplosiveLightEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.EXPLOSIVE_LIGHT_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.EXPLOSIVE_LIGHT_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.EXPLOSIVE_LIGHT_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.EXPLOSIVE_LIGHT_MAX_LEVEL.get();
    }
