...
public class FireflyEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZConfig.FIREFLY_BASE_ENCHANTABILITY.get() + (level - 1) * TBZConfig.FIREFLY_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZConfig.FIREFLY_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZConfig.FIREFLY_MAX_LEVEL.get();
    }
