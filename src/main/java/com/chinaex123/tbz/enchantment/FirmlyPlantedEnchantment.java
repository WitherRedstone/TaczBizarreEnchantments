...
public class FirmlyPlantedEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.FIRMLY_PLANTED_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.FIRMLY_PLANTED_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.FIRMLY_PLANTED_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.FIRMLY_PLANTED_MAX_LEVEL.get();
    }
