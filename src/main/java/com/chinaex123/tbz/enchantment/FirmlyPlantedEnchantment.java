...
public class FirmlyPlantedEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZConfig.FIRMLY_PLANTED_BASE_ENCHANTABILITY.get() + (level - 1) * TBZConfig.FIRMLY_PLANTED_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZConfig.FIRMLY_PLANTED_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZConfig.FIRMLY_PLANTED_MAX_LEVEL.get();
    }
