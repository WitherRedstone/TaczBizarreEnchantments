...
public class FourthTimeTheCharmEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.FOURTH_TIME_THE_CHARM_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.FOURTH_TIME_THE_CHARM_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.FOURTH_TIME_THE_CHARM_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.FOURTH_TIME_THE_CHARM_MAX_LEVEL.get();
    }
    ...
    public boolean canEnchant(@NotNull ItemStack stack) {
        ...
        if (!GunEnchantmentHelper.canApplyEnchantmentToGunWithWeaponType(
                TBZMod.id("fourth_time_the_charm"), stack
        )) {
            return false;
        }
