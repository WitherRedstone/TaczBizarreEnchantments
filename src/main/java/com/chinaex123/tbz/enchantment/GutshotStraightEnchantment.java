...
public class GutshotStraightEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZConfig.GUTSHOT_STRAIGHT_BASE_ENCHANTABILITY.get() + (level - 1) * TBZConfig.GUTSHOT_STRAIGHT_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZConfig.GUTSHOT_STRAIGHT_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZConfig.GUTSHOT_STRAIGHT_MAX_LEVEL.get();
    }
