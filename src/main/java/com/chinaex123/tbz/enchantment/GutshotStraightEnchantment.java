...
/** 直击要害：瞄准可提升身体射击伤害 **/
public class GutshotStraightEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.GUTSHOT_STRAIGHT_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.GUTSHOT_STRAIGHT_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.GUTSHOT_STRAIGHT_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.GUTSHOT_STRAIGHT_MAX_LEVEL.get();
    }
