...
public class HealClipEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZConfig.HEAL_CLIP_BASE_ENCHANTABILITY.get() + (level - 1) * TBZConfig.HEAL_CLIP_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZConfig.HEAL_CLIP_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZConfig.HEAL_CLIP_MAX_LEVEL.get();
    }
