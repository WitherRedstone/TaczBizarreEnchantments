...
public class IncandescentEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZConfig.INCANDESCENT_BASE_ENCHANTABILITY.get() + (level - 1) * TBZConfig.INCANDESCENT_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZConfig.INCANDESCENT_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZConfig.INCANDESCENT_MAX_LEVEL.get();
    }
