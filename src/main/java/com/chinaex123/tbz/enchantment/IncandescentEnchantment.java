...
public class IncandescentEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.INCANDESCENT_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.INCANDESCENT_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.INCANDESCENT_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.INCANDESCENT_MAX_LEVEL.get();
    }
