...
public class MasterOfArmsEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.MASTER_OF_ARMS_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.MASTER_OF_ARMS_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.MASTER_OF_ARMS_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.MASTER_OF_ARMS_MAX_LEVEL.get();
    }
