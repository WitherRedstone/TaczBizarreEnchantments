...
public class MulliganEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZConfig.MULLIGAN_BASE_ENCHANTABILITY.get() + (level - 1) * TBZConfig.MULLIGAN_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZConfig.MULLIGAN_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZConfig.MULLIGAN_MAX_LEVEL.get();
    }
