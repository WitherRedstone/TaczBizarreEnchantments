...
/** 不法之徒：精准击杀可大幅缩短填装时间 **/
public class OutlawEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.OUTLAW_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.OUTLAW_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.OUTLAW_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.OUTLAW_MAX_LEVEL.get();
    }
