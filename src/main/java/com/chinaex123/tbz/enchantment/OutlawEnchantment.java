...
public class OutlawEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZConfig.OUTLAW_BASE_ENCHANTABILITY.get() + (level - 1) * TBZConfig.OUTLAW_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZConfig.OUTLAW_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZConfig.OUTLAW_MAX_LEVEL.get();
    }
