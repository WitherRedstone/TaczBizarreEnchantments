...
public class PersonalAssistantEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.PERSONAL_ASSISTANT_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.PERSONAL_ASSISTANT_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.PERSONAL_ASSISTANT_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.PERSONAL_ASSISTANT_MAX_LEVEL.get();
    }
