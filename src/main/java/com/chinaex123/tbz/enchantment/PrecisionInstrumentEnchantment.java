...
/** 精准工具：造成精准伤害会提升爆头倍率 **/
public class PrecisionInstrumentEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.PRECISION_INSTRUMENT_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.PRECISION_INSTRUMENT_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.PRECISION_INSTRUMENT_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.PRECISION_INSTRUMENT_MAX_LEVEL.get();
    }
    ...
    public boolean canEnchant(@NotNull ItemStack stack) {
        ...
        if (!GunEnchantmentHelper.canApplyEnchantmentToGunWithWeaponType(
                TBZMod.id("precision_instrument"), stack
        )) {
            return false;
        }
