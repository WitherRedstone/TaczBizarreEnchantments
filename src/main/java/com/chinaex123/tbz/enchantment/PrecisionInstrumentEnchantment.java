...
public class PrecisionInstrumentEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZConfig.PRECISION_INSTRUMENT_BASE_ENCHANTABILITY.get() + (level - 1) * TBZConfig.PRECISION_INSTRUMENT_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZConfig.PRECISION_INSTRUMENT_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZConfig.PRECISION_INSTRUMENT_MAX_LEVEL.get();
    }
