...
public class ProtectiveWeaveEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZConfig.PROTECTIVE_WEAVE_BASE_ENCHANTABILITY.get() + (level - 1) * TBZConfig.PROTECTIVE_WEAVE_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZConfig.PROTECTIVE_WEAVE_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZConfig.PROTECTIVE_WEAVE_MAX_LEVEL.get();
    }
    ...
    public boolean canEnchant(@NotNull ItemStack stack) {
        ...
        if (!GunEnchantmentHelper.canApplyEnchantmentToGunWithWeaponType(
                TBZMod.id("protective_weave"), stack
        )) {
            return false;
        }
