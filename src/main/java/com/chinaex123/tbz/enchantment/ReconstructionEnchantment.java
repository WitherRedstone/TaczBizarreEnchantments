...
public class ReconstructionEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZConfig.RECONSTRUCTION_BASE_ENCHANTABILITY.get() + (level - 1) * TBZConfig.RECONSTRUCTION_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZConfig.RECONSTRUCTION_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZConfig.RECONSTRUCTION_MAX_LEVEL.get();
    }
