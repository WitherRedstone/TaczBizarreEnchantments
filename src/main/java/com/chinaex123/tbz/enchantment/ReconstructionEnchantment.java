...
public class ReconstructionEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.RECONSTRUCTION_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.RECONSTRUCTION_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.RECONSTRUCTION_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.RECONSTRUCTION_MAX_LEVEL.get();
    }
    ...
    public boolean canEnchant(@NotNull ItemStack stack) {
        ...
        if (!GunEnchantmentHelper.canApplyEnchantmentToGunWithWeaponType(
                TBZMod.id("reconstruction"), stack
        )) {
            return false;
        }
