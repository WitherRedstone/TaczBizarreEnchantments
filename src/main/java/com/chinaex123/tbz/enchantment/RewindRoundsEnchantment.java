...
public class RewindRoundsEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.REWIND_ROUNDS_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.REWIND_ROUNDS_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.REWIND_ROUNDS_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.REWIND_ROUNDS_MAX_LEVEL.get();
    }
