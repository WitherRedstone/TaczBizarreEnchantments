...
public class RimestealerEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZConfig.RIMESTEALER_BASE_ENCHANTABILITY.get() + (level - 1) * TBZConfig.RIMESTEALER_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZConfig.RIMESTEALER_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZConfig.RIMESTEALER_MAX_LEVEL.get();
    }
