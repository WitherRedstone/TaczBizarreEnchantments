...
public class RollingStormEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZConfig.ROLLING_STORM_BASE_ENCHANTABILITY.get() + (level - 1) * TBZConfig.ROLLING_STORM_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZConfig.ROLLING_STORM_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZConfig.ROLLING_STORM_MAX_LEVEL.get();
    }
