...
public class ShootTpLootEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.SHOOT_TP_LOOT_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.SHOOT_TP_LOOT_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.SHOOT_TP_LOOT_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.SHOOT_TP_LOOT_MAX_LEVEL.get();
    }
