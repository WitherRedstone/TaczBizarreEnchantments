...
public class SliceEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZConfig.SLICE_BASE_ENCHANTABILITY.get() + (level - 1) * TBZConfig.SLICE_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZConfig.SLICE_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZConfig.SLICE_MAX_LEVEL.get();
    }
