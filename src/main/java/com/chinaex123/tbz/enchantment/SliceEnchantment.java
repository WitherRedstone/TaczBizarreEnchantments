...
/** 切割：击中生物时给生物施加瓦解效果 **/
public class SliceEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.SLICE_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.SLICE_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.SLICE_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.SLICE_MAX_LEVEL.get();
    }
