...
public class SnapshotSightsEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZConfig.SNAPSHOT_SIGHTS_BASE_ENCHANTABILITY.get() + (level - 1) * TBZConfig.SNAPSHOT_SIGHTS_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZConfig.SNAPSHOT_SIGHTS_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZConfig.SNAPSHOT_SIGHTS_MAX_LEVEL.get();
    }
