...
/** 速射瞄准：大幅提高瞄准速度 **/
public class SnapshotSightsEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.SNAPSHOT_SIGHTS_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.SNAPSHOT_SIGHTS_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.SNAPSHOT_SIGHTS_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.SNAPSHOT_SIGHTS_MAX_LEVEL.get();
    }
