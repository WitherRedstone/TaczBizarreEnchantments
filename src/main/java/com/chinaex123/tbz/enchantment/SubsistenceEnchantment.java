...
public class SubsistenceEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.SUBSISTENCE_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.SUBSISTENCE_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.SUBSISTENCE_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.SUBSISTENCE_MAX_LEVEL.get();
    }
