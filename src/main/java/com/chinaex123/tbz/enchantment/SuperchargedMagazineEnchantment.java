...
public class SuperchargedMagazineEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZConfig.SUPERCHARGED_MAGAZINE_BASE_ENCHANTABILITY.get() + (level - 1) * TBZConfig.SUPERCHARGED_MAGAZINE_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZConfig.SUPERCHARGED_MAGAZINE_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZConfig.SUPERCHARGED_MAGAZINE_MAX_LEVEL.get();
    }
    ...
    public boolean canEnchant(@NotNull ItemStack stack) {
        ...
        if (!GunEnchantmentHelper.canApplyEnchantmentToGunWithWeaponType(
                TBZMod.id("supercharged_magazine"), stack
        )) {
            return false;
        }
