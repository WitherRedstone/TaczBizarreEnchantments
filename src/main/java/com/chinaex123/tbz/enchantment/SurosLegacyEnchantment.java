...
public class SurosLegacyEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.SUROS_LEGACY_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.SUROS_LEGACY_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.SUROS_LEGACY_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.SUROS_LEGACY_MAX_LEVEL.get();
    }
