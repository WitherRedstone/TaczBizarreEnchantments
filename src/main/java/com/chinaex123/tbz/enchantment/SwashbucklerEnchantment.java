...
/** 斗剑士：击杀生物后增加对目标的伤害 **/
public class SwashbucklerEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.SWASHBUCKLER_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.SWASHBUCKLER_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.SWASHBUCKLER_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.SWASHBUCKLER_MAX_LEVEL.get();
    }
