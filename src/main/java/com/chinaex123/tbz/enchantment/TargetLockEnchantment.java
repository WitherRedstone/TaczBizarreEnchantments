...
public class TargetLockEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZConfig.TARGET_LOCK_BASE_ENCHANTABILITY.get() + (level - 1) * TBZConfig.TARGET_LOCK_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZConfig.TARGET_LOCK_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZConfig.TARGET_LOCK_MAX_LEVEL.get();
    }
