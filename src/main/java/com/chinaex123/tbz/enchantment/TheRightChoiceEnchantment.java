...
/** 正确抉择：每第七发子弹会弹向附近目标 */
public class TheRightChoiceEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.THE_RIGHT_CHOICE_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.THE_RIGHT_CHOICE_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.THE_RIGHT_CHOICE_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.THE_RIGHT_CHOICE_MAX_LEVEL.get();
    }
