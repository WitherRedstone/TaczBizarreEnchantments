...
public class TrickleChargeEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.TRICKLE_CHARGE_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.TRICKLE_CHARGE_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.TRICKLE_CHARGE_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.TRICKLE_CHARGE_MAX_LEVEL.get();
    }
    ...
    public boolean canEnchant(@NotNull ItemStack stack) {
        ...
        if (!GunEnchantmentHelper.canApplyEnchantmentToGunWithWeaponType(
                TBZMod.id("trickle_charge"), stack
        )) {
            return false;
        }
