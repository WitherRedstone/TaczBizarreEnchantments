...
public class TrickleChargeEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZConfig.TRICKLE_CHARGE_BASE_ENCHANTABILITY.get() + (level - 1) * TBZConfig.TRICKLE_CHARGE_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZConfig.TRICKLE_CHARGE_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZConfig.TRICKLE_CHARGE_MAX_LEVEL.get();
    }
