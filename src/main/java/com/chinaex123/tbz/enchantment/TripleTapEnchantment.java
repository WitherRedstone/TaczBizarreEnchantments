...
public class TripleTapEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZConfig.TRIPLE_TAP_BASE_ENCHANTABILITY.get() + (level - 1) * TBZConfig.TRIPLE_TAP_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZConfig.TRIPLE_TAP_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZConfig.TRIPLE_TAP_MAX_LEVEL.get();
    }
