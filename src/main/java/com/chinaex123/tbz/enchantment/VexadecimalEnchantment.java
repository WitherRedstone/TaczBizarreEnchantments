...
/** 十六进制：每4发削弱，每16发增伤 */
public class VexadecimalEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.VEXADECIMAL_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.VEXADECIMAL_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.VEXADECIMAL_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.VEXADECIMAL_MAX_LEVEL.get();
    }
