...
/** 泉源：使用此武器造成击杀会产生经验球 **/
public class WellspringEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.WELLSPRING_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.WELLSPRING_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.WELLSPRING_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.WELLSPRING_MAX_LEVEL.get();
    }
