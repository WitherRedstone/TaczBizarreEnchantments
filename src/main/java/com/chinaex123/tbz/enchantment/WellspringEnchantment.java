...
public class WellspringEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZConfig.WELLSPRING_BASE_ENCHANTABILITY.get() + (level - 1) * TBZConfig.WELLSPRING_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZConfig.WELLSPRING_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZConfig.WELLSPRING_MAX_LEVEL.get();
    }
