...
public class WitheringGazeEnchantment extends Enchantment {
    ...
    public int getMinCost(int level) {
        return TBZServerConfig.WITHERING_GAZE_BASE_ENCHANTABILITY.get() + (level - 1) * TBZServerConfig.WITHERING_GAZE_LEVEL_ENCHANTABILITY.get();
    }

    /** 最大附魔成本 **/
    @Override
    public int getMaxCost(int level) {
        return this.getMinCost(level) + TBZServerConfig.WITHERING_GAZE_ENCHANTABILITY_SPAN.get();
    }

    /** 最大附魔等级 **/
    @Override
    public int getMaxLevel() {
        return TBZServerConfig.WITHERING_GAZE_MAX_LEVEL.get();
    }
