 ...
 * 机制：
 * 1. 玩家射击时（弹药减少）记录射击时间并标记为待判定状态
 * 2. 射击后5刻（0.25秒）内若有命中事件发生，取消弹药返还（说明命中了）
 * 3. 若5刻内无命中记录，触发弹药返还判定：
 *    a. 计算返还概率：基础概率 + (等级 - 1) × 每级增加概率
 *    b. 概率判定成功则返还1发弹药到弹匣
 *    c. 仅当弹匣未满时才返还弹药（防止溢出）
 * 4. 每次射击独立判定，命中优先取消返还
 * 5. 判定完成后清除待判定状态，等待下次射击
 * 6. 返还概率随附魔等级提升而增加
 */
public class MulliganEvent {

    // NBT标签常量
    private static final String SHOT_TIME_TAG = "MulliganShotTime"; // 射击时间记录
    private static final String PENDING_RETURN_TAG = "MulliganPendingReturn"; // 是否等待弹药返还判定
    private static final int CHECK_DELAY_TICKS = 5;  // 射击后等待5刻（0.25秒）再检查是否命中
    ...
    public static void onEntityHurtByGun(EntityHurtByGunEvent.Pre event) {
        ...
        if (shotTime != null && player.level().getGameTime() - shotTime < CHECK_DELAY_TICKS) {
            ...
            tag.putBoolean(PENDING_RETURN_TAG, false);  // 取消待处理的弹药返还
            lastShotTimeMap.remove(playerId);           // 清除射击记录
        }
    }
    ...
    public static void onPlayerTick(Player player) {
        ...
        if (prevAmmo > currentAmmo && prevAmmo > 0) {
            lastShotTimeMap.put(playerId, player.level().getGameTime());  // 记录射击时间
            tag.putBoolean(PENDING_RETURN_TAG, true);                      // 标记等待返还判定
        }
                ...
                if (player.getRandom().nextFloat() < totalChance) {
                    ...
                    if (gunIndexOpt.isPresent()) {
                        ...
                        if (currentAmmo < magazineSize) {
                            iGun.setCurrentAmmoCount(gun, currentAmmo + 1);  // 返还1发弹药
                        }
                    }
                }
